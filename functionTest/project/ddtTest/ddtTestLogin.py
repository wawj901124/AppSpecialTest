...
class TestLogin(unittest.TestCase):  # 创建测试类
    ...
    #登录的测试方法
    def definelogin(self,outtestcasediscription=None,isStartApp=False,isclickloginwithpassword=False,outphonenumberinput=None,
        ...
        if isStartApp:
            self.baseframe.startapp(AppPackageName)
        if isclickloginwithpassword:
                self.baseframe.findbyresourceId_and_click(OTPLogInWithPasswordId)
        self.baseframe.findbyresourceId_and_input(AccountInputId, phonenumberinput)  # 输入账号
        self.baseframe.findbyresourceId_and_input(PasswordInputId, pwdinput)  # 输入密码
        ...
        if outexceptiontext == 'OK':
            print("")
        elif outexceptiontext == 'Login':
            eleenable = self.baseframe.findbytext_and_return_enabledstatus('Login')
            self.assertFalse(eleenable)
        elif outexceptiontext =='Try again':
            self.baseframe.findbytext_and_click(outexceptiontext)

        print("%s.---测试通过" %testcasediscription)

    @data(("账号密码错误提示",True,True,"81234567891234","123456","Incorrect account number or login password. Please try again(PP001)",None,"Try again"),
          ("登录账号长度小于8位，login按钮置灰", False, False, "1234567", "123456","Login", None, "Login"),
          ("密码长度小于6位，login按钮置灰", False, False, "12345678", "12345", "Login", None, "Login"),
          ("非8或08开头的账号提示非印尼号", False, False, "12345678", "123456", "Login", "You have entered an invalid Indonesia number", None),
          ("正确的账号密码登录成功",False,False,"81122336666","abc123456","Show all",None,None))
    @unpack
    def test_login(self,outtestcasediscription,isStartApp,isotplogin,outphonenumberinput,
                    outpwdinput,outprepagetext,
                    outpretoastmessage,outexceptiontext):
        self.definelogin(outtestcasediscription,isStartApp,isotplogin,outphonenumberinput,
                    outpwdinput,outprepagetext,
                    outpretoastmessage,outexceptiontext)
