...
class Controller(object):
    def __init__(self):
        ...
        self.alldata = [("deviceid","appversion","timestamp", "meminfosize")]  #  要保存的数据，时间戳及cup占用率

    #单次测试过程
    def TestProcessOnce(self):
        cmd = 'adb shell "dumpsys meminfo | grep %s"' % AppPackageName   # 获取cup占用率命令
        ...
        if result != []:
            for line in result:
                ...
                if meminfosize:   #如果取到值，就终止循环
                    print("获取到内存为：%s kB"% meminfosize)
                    break
            currenttime = self.GetCurrentTime()  #  获取当前时间
            self.alldata.append((TestDeviceID,AppVersion,currenttime,meminfosize))  #  写入数据到self.alldata
        else:
